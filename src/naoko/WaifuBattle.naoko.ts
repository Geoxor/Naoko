...
import { calcDamage, calcParticipated } from "../logic/logic.naoko";
import { User } from "./Database.naoko";
import logger from "./Logger.naoko";
import Waifu from "./Waifu.naoko";
...
export default class WaifuBattle {
  ...
  public async endBattle() {
    ...
    setTimeout(async () => {
      try {
        this.channel.name === "naoko" ? await this.thread!.setArchived(true) : await this.thread?.delete();
      } catch (error: any) {
        logger.error(error);
      }
    }, this.AFTERMATH_TIME);
